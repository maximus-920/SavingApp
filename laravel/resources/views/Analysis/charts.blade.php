...
<head>
    ...
    <link rel="stylesheet" href="{{asset('assets/css/chartsStyle.css')}}">
</head>
<body>
<!-- ナビゲーションバー -->
...
<div class="container-fluid body pb-5">
    <div class="row">
        <div class="col-6">
            <div class="card-body"  style="position: relative;width: 45rem; height: 35rem;">
                <h5>Expense transition</h5>
                <canvas id="myChart"></canvas>
            </div>
        </div>
        <div class="col-6">
            <div class="card-body" style="position: relative;width: 45rem; height: 35rem;">
                <h5>Income transition</h5>
                <canvas id="chart2"></canvas>
            </div>
        </div>
        <div class="alert alert-success ml-5 mb-5" role="alert">
            Your total saving of this month is ${{$saving}}. Your goal saving of this month is ${{$goal_saving}}
        </div>
    </div>
</div>
...
<script>
    ...
    new Chart(ctx, {
        ...
        data: {
            ...
            datasets: [{
                label: 'Monthly expense in doller',
                data: [@foreach($monthlyExpenses as $me)
                    '{{$me}}',
                @endforeach],
                borderWidth: 1
            }]
    ...
    new Chart(ctx2, {
        ...
        data: {
            labels: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'],
            datasets: [{
                label: 'Monthly income in doller',
                data: [@foreach($monthlyIncome as $mi)
                    '{{$mi}}',
                    @endforeach],
                borderWidth: 1
            }]
